...
const App: Component = () => {
	...
	const initWs = () => {
		const newWs = createWS(wsUrlBase + "/ws?id=" + roomCode);
	...
	return (
		...
		>
			<div id="content">
				<div id="banner">
					<div>
						{connectedClients() == 1 ? (
							<p>
								Open{" "}
								<a href={origin + "/" + roomCode}>
									{origin}/{roomCode}
								</a>{" "}
								on another device to copy/paste between devices.
							</p>
						) : (
							<p>
				...
				<div>
					<Clipboards entries={Object.values(clipboards())} />
				</div>
			</div>
		</div>
	);
