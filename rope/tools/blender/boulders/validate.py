...
# Constructions that build a closed 3D body around the centre plane, as opposed
# to the legacy relief panel: boulder v5's "volume" and dirt/moss's "sdf".
VOLUMETRIC = {"volume", "sdf"}
...
def projected_union(vertices, faces, threshold=None, grid=1e-7):
    ...
    def clean(geometry):
        ...
        return geometry
    # The same overlay leaves zero-width slits where a sliver triangle at a
    # fold drops out of the union; unlike the rings above they open onto the
    # outline, so they read as the silhouette falling far short of it. A
    # closing a hundred grid cells wide seals them and moves real edges by no
    # more than that.
    seal = grid * 100
    return clean(joined).buffer(seal, join_style="mitre").buffer(-seal, join_style="mitre")
...
def validate_mesh(data):
    ...
    eps = spec["tolerance"]
    if spec.get("construction") in VOLUMETRIC:
    ...
    result = {
        ...
        "front_measurement_depth": None if spec.get("construction") in VOLUMETRIC else spec["depth"] * 0.51,
...
def render_outline(p, actual, upper, result, path):
    ...
    axes[1].set_title("Visible surface coverage" if result.get("construction") in VOLUMETRIC else "Front relief coverage", loc="left")
