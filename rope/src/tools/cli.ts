...
async function cmdClicks(file: string, all: boolean, evdevFile?: string, waylandFile?: string): Promise<void> {
  const { auditClicks, auditSummary, parseEvdev, parseWaylandDebug, evdevReport } = await import("../input/inputTrace");
  const rec = loadRecording(file);
  if (!rec.inputTrace) {
  ...
  // The mouse's own stream beside the browser's (`libinput debug-events`), and
  // the compositor's (`WAYLAND_DEBUG=1` on the browser), each aligned to the
  // trace by the clicks they share.
  if (evdevFile) {
    for (const line of evdevReport(rec.inputTrace, parseEvdev(readFileSync(evdevFile, "utf8")), "evdev")) {
      console.log(`  ${line}`);
    }
  }
  if (waylandFile) {
    for (const line of evdevReport(rec.inputTrace, parseWaylandDebug(readFileSync(waylandFile, "utf8")), "wayland")) {
      console.log(`  ${line}`);
    }
  }
...
async function cmdPull(o: Record<string, string>): Promise<void> {
  ...
  //
  // The bipolar difficulty scale drawn as a bar with the answer's side on it,
  // fixed width so a column of them reads as a shape.
  const DIFFICULTY_MARKS: Record<number, string> = {
    1: "<<   ",
    2: " <   ",
    3: "  |  ",
    4: "   > ",
  ...
  const said = (await fb.json()) as {
    feedback: {
      ...
      player: string; stars: number | null; difficulty: number | null; comment: string | null; at: number;
    }[];
    players: Record<string, { name: string | null }>;
  };
  ...
  for (const r of said.feedback) {
    ...
    // The bipolar difficulty as a SIDE and a distance, which is what it is: one
    // mark either side of a bar that is "just right" (see `Difficulty` in
    // playtest/feedback.ts). A column of `<<`, `<`, `|`, `>`, `>>` can be read
    // straight down for whether a level is missing, and which way - which a
    // column of 1..5 cannot.
    const diff = r.difficulty == null ? "  -  " : DIFFICULTY_MARKS[r.difficulty] ?? "  ?  ";
    ...
    console.log(
      `  ${when}  ${who.padEnd(12)} ${r.level.padEnd(8)} v${seq.get(r.id)}/${totals.get(k)} ${stars} ${diff} @${r.commit.padEnd(7)} ${level.padEnd(20)} ${r.comment ?? ""}`,
    );
  }
}
...
// Scripted-mover cases (src/sim/moverCases.ts). Pure physics like `spring`, and
// separate from it for the same reason: what a mover does has a CLOSED FORM -
// the pendulum is a sine about its bearing, the travelling body is a distance
// along its route - so these assert arithmetic an author reasons about while
// timing a jump, plus the four claims that make a mover a mover (it cannot be
// disturbed, it carries its rider, it is a pure function of the frame, and every
// authored field is actually read).
async function cmdMovers(): Promise<void> {
  // Dynamic for the same reason as `cli contacts`: the case file reaches into
  // engine internals an old revision does not have, and `cli compare` runs this
  // very file inside a worktree of one.
  const { runMoverCases } = await import("../sim/moverCases");
  const results = runMoverCases();
  let failed = 0;
  for (const r of results) {
    console.log(`  ${r.passed ? "PASS " : "FAIL "} ${r.name}`);
    for (const d of r.details) console.log(`        ${d}`);
  ...
  console.log(`[movers] ${results.length - failed}/${results.length} cases passed`);
  process.exit(failed > 0 ? 1 : 0);
}
