...
And the span that ends on the ring ignores, in the wrap scan, every piece of the bar's body inside the cuff's own disc (`insideCuff` in `regeneratePath`): the chain leaves the ring at its rim, so the neighbouring pieces of the bar the ring straddles and the lid it hangs beside are metal the chain is already clear of, not corners to bend round - wrapping the lid 12 mm from the ring's centre gave a last span shorter than the ring is wide, whose direction the ring's own tilt changed, and the ring hunted between that and gravity while the drawn cuff jumped about the handle (`session-189f`).
...
The chain's last link is hooked over the END of that ring (`cuffRimDirection`) rather than run to the drape's own direction: a cuff drawn edge-on has metal at the two ends of its long axis and hole everywhere between them, so a link laid `chainDir` from the centre is a link attached to the air inside the ring.
