...
The rollback reaches every body but the ball **except those of a coupled set the sweep has CONVERGED**, and "converged" is the operative word rather than "chain-held".
Where the sweep really has settled a set it has already apportioned the correction between the ball and the anchor and closed its own books over the displacement it left (`settleChainBodies`), so rolling one end back does not un-export the spin - it re-breaks a constraint the sweep had just satisfied, and the unwind charges that over-length to the ball's rotation a second time.
That is a vine, and only a vine: `CoupledRope.settleSet` is the sweep's own statement about which set it holds to a tolerance, and it is true while a vine is held and false for an ordinary scene set, which is gated on the coupling's residual alone and left over its own tolerance on 1616 frames of 1618.
Excluding chain-held bodies on that strength handed a hanging anchor the whole of a rotation the unwind then refused in full: the aim wound 0.55 rad a frame onto a ball whose net turn was exactly **zero**, and the anchor was hauled and paid for the winding every frame of it, the pair accelerating together from 3 to 25 m/s over 35 frames while the stall lease let the chain out from 1.18 m to 1.80 (`session-215f`, reported as being flung across the level on winding up into a body suspended on a chain).
It is `session-265f`'s failure exactly, wearing the one mounting that fix did not cover, and it replays HEALTHY - the energy monitor disarms on any frame the aim is turning the ball, which is every frame of it.
`cli contacts` `hung-anchor` is the detector: the same half-metre weight bolted to the ceiling and hung from it, whipped with identical inputs, 1.6 m/s against 8.5 with the weight dragged off at 9.6.
The vine half is `cli vines` `ball-steer`, which reads the exclusion's worth directly - 5.7 degrees of loop lag with the vine excluded, 10.6 without.
