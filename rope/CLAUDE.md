...
"Pulling" is read with a margin: the ring's own tilt moves its centre by up to a bore's radius, so a chain exactly at length goes slack by that much as the ring swings toward the pull, and read as slack-or-not it hunted between the pull and gravity every frame; the ring follows the pull unless the chain is slack by more than the ring itself can make.
And the span that ends on the ring ignores, in the wrap scan and in the start-of-span resolver, every CORNER of the bar's body inside the cuff's own disc (`inCuff` in `regeneratePath` and `resolveSelfIntersectionAtStart`): the chain leaves the ring at its rim, so the joint of the bar the ring straddles and the near corner of the lid it hangs beside are metal the chain is already clear of, not corners to bend round - wrapping the lid 12 mm from the ring's centre gave a last span shorter than the ring is wide, whose direction the ring's own tilt changed, and the ring hunted between that and gravity while the drawn cuff jumped about the handle (`session-189f`); the corner and not the piece, because the far corner of that same base is what the chain bends round when the ball winds up under the lantern (`session-154f`).
