...
bun run generators:setup   # once per machine: the venv the editor's rock generator needs (Blender 5.2 on PATH too)
```
...
`/editor` is the level editor; **W** switches it between the Level workspace and the Visuals workspace (a free 3D view where rocks and mushroom patches are generated by Blender behind the dev server, after `bun run generators:setup` once per machine; [docs/editor-visuals.md](docs/editor-visuals.md)).
...
- [editor-visuals](docs/editor-visuals.md) - the Visuals workspace (W): switching, the free view and its gestures, the guides and their pick tags, picking and editing in the scene, drop on surface, the tools offered and not, + Rock and + Mushrooms and their panels.
- [generators](docs/generators.md) - the dev-server generator service (endpoints, jobs, supersede, failures, timings), the parameter schemas, the ported boulder and mushroom Python, `generators:setup`/`check`, adding a parameter or a generator.
