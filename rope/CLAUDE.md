...
An iteration that ends longer than it began is undone and ends the pass, and what stands is over-length for the unwind and the lease, as a correction geometry refuses always was.
...
Once the unwind gives back 90% or more of a turn, the steering may turn that way only as far as the chain's own length allows, `maxRopeLength` less the path, with no lease counted, until the player aims the other way; a decaying memory of the refused spin was tried first and converges to demanding half the command every frame.
