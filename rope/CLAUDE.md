...
It seeds the last flight step at a span of 1.6005 - chain-out at `t = 0.9975`, inside the step and past the 1.8105 the sweep's own reach covers - rather than flying a whole throw, because a natural throw can only express the tie itself; a ceiling a hair inside the hook's own radius past full stretch must anchor, one a hair outside it must not, and the anchored path must stay inside `CHAIN_MAX_LENGTH + 2 * hook radius` with the chain never dropped.
The `chain-out` case carries the other half at throw scale: a ceiling just inside the band anchors, one comfortably past it does not.
**Both cases derive those placements from `MANACLE_DISC` rather than writing them down**, and the two are measured a radius apart because they are measured to different things - `hook-snap-band`'s gap is to the hook's CENTRE at contact, so its band is one radius wide, while `chain-out`'s is to the ceiling's FACE, which the anchor is placed on, so its band is two.
Written as literals against a 20 mm cuff (15/21 mm and 10/50 mm) every one of them was inside both bands at 53.5 mm: the "out of range" ceiling was legitimately caught, and the hook-proof wall the chain is supposed to stop short of was bounced off before chain-out, so the case's jerk was never measured at all.
...
Read live off the clamp rather than from the frozen `anchorFacingLocal`, which on a curved handle is the tangent the bar had where the hook first STRUCK it.
...
The chain's last link is hooked over the END of that ring rather than run to the drape's own direction: a cuff drawn edge-on has metal at the two ends of its long axis and hole everywhere between them, so a link laid at the chain's direction from the centre is a link attached to the air inside the ring.
