...
What keeps the wound-tight regime honest for such a holder is the pair push-out (`separateBallFromPathBodies`): both bodies hauled into each other are pushed back by the shares they were hauled in by, so the length the unwind then refuses is the whole of the winding and neither is credited a thing - which is the statement `session-215f` needed, made in position rather than by rolling the anchor back.
