...
- **rclone is not installed on the host.** The cron runs the pinned `rclone/rclone` image with the store and the config mounted read-only. The first two deploys failed on apt's lists lock, held for minutes by unattended-upgrades, and a deploy must not depend on the host's package manager being idle.
- **Feedback is stored beside the runs and is not one.** Five stars and a comment, left at the finish line that ends a level, append-only in `<dir>/feedback.ndjson` and sharing the `pid` cookie so a player's runs and their ratings are one player. See [**Levels**](../docs/levels.md#the-feedback-form) for the whole of it; nothing about it changes the run path.
