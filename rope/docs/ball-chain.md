...
The rollback reaches every body but the ball - save the free holder the ball is not riding, and every holder of a braced ball, below - and the whole of the difficulty is what the frame then does with the length the rollback re-creates.
...
A free body the ball is riding in free air, a pivot or a sprung mount is answered by the unwind: there the re-break IS the winch's governor, and weakening it is what ran away on `session-136f` and on `whirl-anchor`.
...
The winch is owed only for such a body ON THE PATH - a chain-held body elsewhere in the level is not what re-broke the ball's constraint (`session-155f`, below).

A **free rigid body** the ball is hauling on across open air is answered by neither, because for it the rollback's premise is simply false (`BallLevel.keepsHaul`, 2026-09-12 for the chain-held holder, 2026-09-16 for any).
...
Open air is the premise of the whole rollback, and a **braced** ball is outside it (`BallLevel.chainBraced`, 2026-09-16).
Every runaway above is a ball with nothing to push against but the chain's own far end: the spin's reaction can only reach the anchor through the chain, so whatever the anchor keeps of it is momentum from nowhere.
A ball carrying a load-bearing contact this frame against something that is NOT on the chain's path has a reaction the world supplies, and there the premise is simply false: the same kinematic spin the contact solve already lets drive the ball along the ground at rim speed drives the chain onto the rim against that same ground, and a free body on the far end of the chain is hauled by it, riding the ball or not.
It is bounded the way a winch is bounded - the solve's correction is the frame's winding, so nothing on the path is credited more than rim speed - and the pair push-out and the statics refusal stand between the haul and any overlap it creates.
`session-427f` is the recording: the ball resting on a slope beside a 25 kg stool it is hooked to, winding on with the stool against it, and the rollback stripping the stool's share on every taut frame (rope-solve +0.35 m/s, spin-rollback -0.35 at f352, the whole of it at f353-357), so the stool crept on the leftovers of the frames whose over-length counted as real motion, the ball wound tight, and the stall latch closed with 6 cm on lease.
Kept, the same recorded inputs pull the stool up and over the ball onto its other side; `playtests/rigs/braced-box-wind.json` is the arena as an instrument - a 25 kg box on a flat floor, the ball hooked to its near top corner and wound at 90 frames a revolution - and it reads the same: the box dragged at 0.57 m/s up to the ball, then climbing it and landing upside down on the far side, 2.5 m/s at the peak and nothing over length.
Pivots, spring mounts and vine links keep their own governors either way.
The gate is the one `keepsHaul` already had, and the same `braced` flag is what ends the wind-stall latch below; both are read straight off a bundle as `braced=` on the dump's chain line.

The first cut kept the free-air share only for a holder a scene chain holds, and `session-155f` (the same day) is what a free holder in free air costs: the braced haul threw the 25 kg stool up and over at rim speed - 3.3 m/s at a 29 rad/s ask, the bound doing exactly what it says - and the moment the ball's own share lifted it off the floor the old rule took over, the stool was rolled back, and the ball was hauled ALONE after a 25 kg body flying at 4 m/s as though it were bolted to the sky: 9.7 m/s in sixteen frames, 500 kg m/s from nowhere, the stool's own speed untouched (f110-126), replayed HEALTHY.
Two things were wrong, and the second is older than the first.
A free body the ball is hauling on across open air now keeps the coupled share whether a scene chain holds it or nothing does - the argument that a body held by nothing answers only through contacts the solve cannot see is answered where it was answered for the chain-held plank, by `refuseRopeBodiesIntoStatics` on the same `pathBefore` snapshot and by next frame's contact solve - so the ball closes on it at the share the masses say.
And the winch pass after the rollback is owed only for a rolled-back chain-held holder ON THE PATH: the gate used to ask whether any such body existed at all, over `haulAtSolve`, which is every rigid body in the level, so in an arena with eleven scene chains it was true on every frame the ball rode its anchor in free air whatever the anchor was, and it was that pass, hauling the ball alone with the stool held immovable, that wrote the 44 mm a frame.
Scoped to the path, the same recording hops the ball 55 cm and drops it back at 1.1 m/s while the thrown stool lands on the far side; `session-427f`'s own recorded inputs now pull the stool over the ball as asked, with the ball peaking at 2.9 m/s.
What is deliberately left as it reads is the throw itself: a braced ball asked for 29 rad/s hands a 25 kg stool rim speed, which is the leverage that was asked for at its limit, and whether that limit wants a softer aim is a question for play, not for the solver.

The throw exposed the last free-air gap, and it is the rollback's own (`session-325f`, the same day, replayed HEALTHY): a 25 kg stool thrown up by the braced haul with the ball wound point-blank onto it, both in the air for 36 frames.
The stool was in plain free fall - its solve share of +12 mm a frame rolled back to the digit, as a ridden holder in free air is - while the ball's own share of the winding stood, 5.5 mm a frame toward the stool credited at 0.33 m/s each, so the ball winched itself toward a body that never answered: it rose 0.9 m against gravity, swung round the stool at 3.7 m/s and landed at 5.7.
Keeping the holder's share there instead was measured first and pumps the way `session-215f` does (the stool thrown at 5.3 m/s, the ball still lifted 0.7 m).
So when the ridden holder is held by NOTHING - not the ground, not a scene chain, not another body: no load-bearing contact off the ball - and the ball has no brace of its own, the ball's share goes back with the holder's (`ballAtSolve`), the phase-end credit reads the displacement that survived, which is none, and the unwind hands the frame's turn back: nothing in the air can wind against a body nothing holds.
A holder on the floor, on a chain or leaning on anything else still gives the ball its winch exactly as before - that support is what a wind-up onto a crate on a ledge pulls against - and the same recording now lifts the ball 0.4 m at most and has it climb onto the stool once the stool has landed.

The ball's share goes back for a second reason too: a BRACED ball with the free holder riding it (`session-179f`).
A winch drum bolted to the ground by its brace does not move toward the load it is hauling over itself, but the solve splits the winding by inverse mass - a third to the ball toward the holder's anchor, two thirds to the holder toward the rim - and the pair push-out then splits their overlap along the CONTACT normal.
Where the chain and the contact are not collinear, a stool sitting on the ball and hooked at a corner, the difference leaks out as motion every frame: 3.4 mm right and 6.8 mm up on the ball, 1.2 and 2.8 back, a net hop of 4 mm a frame that outran gravity, unloaded the floor, and let the 52 kg ball creep 7 cm to the right under the stool going over it with friction reading nothing to hold (f134-170, felt as the ball being pushed back).
So while braced and ridden the holder keeps its share and climbs, the ball's own share is re-broken length the unwind hands back, and the ground supplies what the drum needed, which it always could; the recording keeps the ball loaded on the floor throughout and rolling slowly with its kept spin.
...
And the latch is a free-air statement: a **braced** ball (see the rollback, above) never latches, and bracing releases a latch already closed (2026-09-16).
Held by nothing but the body it is wound up to, a refused turn will be refused again next frame by the same geometry, and asking on is churn; braced, the holder keeps the haul, and a turn refunded whole is the frame the haul JAMMED - the free body dragged up against the ball, its base stopped by the ball's own side while the chain pulls its top over - not the frame it ended.
Each frame from there the solve lifts the holder a few millimetres up the rim, the pair push-out and the statics refusal take the rest back, and the unwind refunds the ask less that progress; latched on the first such frame, the spin was dead for as long as the two touched and the holder never climbed (`session-427f`; `rig-braced-box-wind` closes the latch on f72, the frame the box arrives, and with the gate the box climbs the ball over the next sixty).
Released rather than merely withheld because a haul up a slope hops the ball off the floor for a frame or two, the latch closed on that frame, and it held with the ball back on the ground and its spin dead (the recording's continuation, f497).
A static anchor point-blank has never latched - `windStallHeld` asks for a rigid body - and runs its wound-tight unwind for hundreds of resting frames (`session-726f`); a braced ball against a rigid holder is that case with a holder that can move.
The latch was also doing a second job nobody had written down: with the steering silenced, the contact solve had no refused spin to sell as roll, and an unlatched braced ball asking on every frame was driven along the floor at rim speed with zero net spin, the stool riding along (`session-518f`); the refused share of each ask now withholds the next frame's drive instead (`RigidBody2D.spinDriveShare`, see [ball-rolling](ball-rolling.md)).
