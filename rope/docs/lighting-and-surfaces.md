...
- **Light falls in bands.** Lambert's `dot(N, L)` is a smooth cosine, so two facets a few degrees apart merely shade and a ball is a smooth gradient - the roundness of a photograph. `paintBands` cuts the wrapped cosine into four bands with a soft edge between neighbours: a facet (a plateau of one normal in the painted maps) lands wholly in one band and is one tone, and a sphere crosses the bands as a lit crescent, a mid tone and a shadow tone with a soft turn between them. The **hemisphere fill's** sky-to-ground blend is cut by the same bands, because in a level lit mostly by fill (the ball arena's fill is twice its sun) that is where most of the tone is; the environment map's irradiance is left smooth, being a fraction of the light. A band's value is the cosine at its middle, so the bands average to the line they replace and levels are not re-lit.
...
- **Nothing is glossy.** Roughness has a floor of 0.5, which widens every highlight into a wash and blurs the environment a metal reflects into a soft tone rather than a mirror of the sky. A floor rather than a scale on the specular term, because a metal's whole colour IS its specular - scaled down, the ball goes black - where a rougher metal is the same metal, matte.
