...
**Painted surfaces.** Since 2026-09-17 the game is a digital painting rather than a photograph (the water went first, see [**Water**](water.md)), and every authored map is **painted on its way through the pipeline** rather than filtered on screen: `--paint <px>` on `assets:optimize-texture` flattens the map into brush-sized patches at the output size, and the flags used are recorded on the map's manifest entry (`TextureMap.paint`) beside its sha256, for the reason a prop's `--simplify` is - a painted map and one painted by hand are the same file.
Painting in texture space rather than as a post-process is the whole decision: a Kuwahara over the finished frame is pixel-aligned, so in a game whose camera never stops panning the world swims under strokes that stay put, thin things (the chain, a vine) smear, and the canvas MSAA is given up for a render target.
Strokes baked into the maps are locked to the surface and move with it, and they cost nothing per frame.
The flattening is a **mean shift** (`-mean-shift`, window = the brush, 10% tolerance): every pixel walks to the mean of the nearby pixels within a tolerance of its own colour until it stops moving, so a region of near-one-colour collapses to exactly one colour - a **plateau** - and the edge between two plateaus stays exactly where it was.
On the **albedo** that is flat planes of tone with crisp breaks, pushed a little further apart by a sigmoidal contrast; on the **normal map** it is **facets** - regions of one normal turning sharply into the next - and under the sun the facets are the flat planes of tone a painter lays down.
This is most of the look, and it is why the albedo alone came out as mud on the first try (dark rock's albedo is nearly flat brown, and every facet the reference showed was lighting).
**Roughness**, **metallic** and **AO** get the same plateaus, so a facet is matte or glossy as one plane.
It was a Kuwahara first, run cheaply over the map shrunk by the brush and grown back, and that read as **blotchy, blurry and flat** in the game: a Kuwahara's patches are its own window - soft dabs the size of the radius wherever the picture is, whatever the picture's structure - and the grow-back blurred every edge, while a wash blur on the roughness made every surface look wet.
The reference art's facets are large and sharp-edged, and the mean shift finds the picture's own regions at whatever size they are and draws their edges sharp.
It runs on the map shrunk - half size up to a brush of 30, smaller beyond so the window never exceeds 15 pixels (its cost is window area times pixels times iterations, minutes per map at full size) - and is grown back with a Catmull-Rom resize, which stays sharp across a plateau edge.
A mean shift never merges a small island whose colour is far from its surroundings however wide its window, so the avatar's rust flecks survived a brush of 50 exactly as they had a brush of 20 - photographed rust on a painted ball.
`--soften <sigma>` (recorded as `paint.soften`) is a Gaussian blur before the flattening for a map whose small detail must go rather than be kept flat: blurred, the flecks dissolve into the plate and the blooms are what is left to flatten. The rusted iron carries 12; rock carries none, its detail being the facets the brush is there to keep.
Three albedo-only treatments finish it: `--cavity <ao-file>` multiplies in the AO's deepest tenth as crisp dark lines, which is how rock carries its fissures the way a painter draws them (and why moss must NOT have it - moss is raised, so its AO is dark exactly where the moss is, and the multiply turned the ground brown); `--saturate <pct>`, because a photo of a thing is duller than a painting of it (125-140 is the range in use); and `--tint <#hex>@<pct>`, which pulled `mossy ground`'s dirt back from orange to olive once saturated without touching its moss.
The brush is chosen per set: 30 px on rock and ground, 40 on `marble cliff` whose veins a smaller window leaves as dots, and 20 on `rusted iron` so the avatar's rust survives as flat blooms rather than merging into the plate.
The three rock sets also carry `normalScale: 1.6`, because the plateaus are the scan's own normals and a plate's tilt only a few degrees between facets: pushed apart, two facets are a stroke apart in tone under the sun, the way the reference draws them.
The **generated** surfaces are painted at source rather than filtered: `paintField` replaces the fractal noise with a lattice of irregular patches, each one tone with a gentle gradient across it, a dark seam drawn between them in the albedo, and a normal built from each patch's own tilt rather than from the height differenced - because the height steps at every seam, and a step differenced is a bevel that turned the first attempt's wood into chocolate tiles (see [**Surfaces**](lighting-and-surfaces.md#surfaces)).
Painted maps are also a fraction of the bytes: the six sets `levels/ball.json` wears went from 13.5 MB to 5.2 MB, because a plateau compresses and grain does not.

**Brush strokes** are the pass after the flattening, once per set that records `strokes: { width }` (`scripts/stroke-textures.ts`, run by `assets:paint`).
It is stroke-based rendering of the ordinary kind: a few thousand elongated dabs, each one flat colour taken from the flattened picture under its centre, laid along the picture's own edges where it has them and along a slow flow field where it does not, and **clipped where the colour under the stroke changes**, so a stroke runs along a crack and never across it and a moss blob keeps its edge.
One dab layout for the whole set is the reason it is a set pass rather than a per-map flag: the albedo, the normal map, the roughness and the AO wear the same strokes, each sampling its own value under the dab, so a stroke is one plane of colour, one facet and one sheen at once - what a stroke of paint is.
Every dab is a little lighter or darker than what it covers, drifts a little warm or cool, tilts its facet a little and is a little more or less matte, because a painter never mixes the same tone twice - and because without that a dab whose colour is the mean of a flat plateau, painted back onto the plateau, is invisible: the first run of the pass returned the albedo byte for byte and said so (its log line reports how many levels the albedo moved).
The strokes go over lossless intermediates and the albedo is encoded lossy once, after them.
The avatar's `painted steel` is not made this way - its strokes are baked from nothing by `scripts/bake-strokes.ts`, there being no photograph worth keeping under them - and the generated surfaces are not stroked yet.

**The bake is reproducible from the manifest alone**, and adding a texture is three steps.
Each map records `raw` - its source image under `assets-src/` - and, for a scalar map, `channel`, beside its `paint`; with those, `bun run assets:paint` (`scripts/paint-textures.ts`) rebuilds every shipped map from its raw by exactly the recorded recipe, in parallel, and then compares each result's sha256 and size with the manifest and prints the lines to paste for any that differ.
It never edits the manifest: the hash and the size are pasted by the person who changed the recipe, at the one moment they should be looking at what changed.
`--check` exits 1 on any difference, `--publish` uploads every map that changed, and a key narrows it to one set.
So a new texture is:

1. Put the raw images under `assets-src/<set>/`.
2. Add a `TEXTURE_ASSETS` entry whose maps name `file`, `raw`, `channel` where the number is not in red, and `paint` (a brush of 30 for rock and ground, `cavity: true` on a rock's albedo, `saturate` 125-140), with `sha256: ""` and `bytes: 0`.
