...
It runs at half the output size (its cost is window area times pixels times iterations, minutes per map at full size) and is grown back with a Catmull-Rom resize, which stays sharp across a two-pixel plateau edge.
