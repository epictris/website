...
A `belt` is a conveyor (see [**Conveyor belts**](conveyors.md)) and holds `wheels` (each an `EdWheel`: a centre `c` in the item's frame and a radius `r`, wheel 0 at the origin, the item's own `pos`), the band's `thickness` and the signed `speed` in m/s; `edShape` and `shapeOf` convert it to and from the file's `{ kind: "belt", wheels: [{ x, y, r }], thickness, speed }` field for field, and `cli render3d`'s `belt:` round trip holds that, the wheel list and the matched twin included.
...
`pointInBody` picks a belt where it collides - on the band or inside a wheel's disc - and not in the hollow between the wheels, where the props standing for them go.
...
A matched geometry twin mirrors the speed and the thickness as well as the wheels, since a drawn belt running at a different speed from the one it collides as would show a tread that lies about the carry; the twin's own `depth` is the belt's `width`, which the belt panel edits on the collision object's behalf.
