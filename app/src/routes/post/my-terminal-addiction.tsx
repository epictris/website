      ...
      <article class="post-body">
        ...
        <p>
          ...
          <pre>
            ▌ hello_world.txt
            <br/>
            <span style="background:#444;">▌ hello.txt</span>
            <br/>
            {'>'} hlo
          </pre>
          <pre>
        ...
        <p>
          To open a file in nvim (neovim), you need to pass the relative path to the file as the first argument. <br/>
          <pre>
            $ nvim filename.txt
          </pre>
          This quickly starts to feel cumbersome when you can't remember the exact name of the file, or it's buried in a deeply nested directory - if only there was a way to quickly search through all the files in the current directory...<br/>
          <pre>
            $ nvim $(fzf)
          </pre>
          This is called a "command substitution". It allows the <code>nvim</code> command to receive the output of the <code>fzf</code> command as its first argument. When you run this command, you'll launch a fuzzy search over all the files in the current directory, but when you press Enter, the selected file will immediately be opened in <code>nvim</code>. I use the following terminal alias daily - it's just a more robust version of the previous command.<br/> 
          <pre>
