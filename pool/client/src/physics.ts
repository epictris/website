...
export const POCKET_LIST: Pocket[] = POCKETS.map((pk) => {
  ...
  const push = 0.02; // recede the hole from the (inset) felt edge
  const hole = 0.04;
...
// One straight run of the cushion polygon in ball-CENTRE space, precomputed with
// its inward unit normal (toward the playfield) and unit tangent so collision is
// a plain swept point-vs-segment test that works for the angled facings too.
...
const LC = 0.12; // long-rail nose start, measured in from the corner (x)
const SC = 0.11; // short-rail nose start, measured in from the corner (y)
const SIDE_HALF = 0.07; // side-pocket mouth half-width (nose ends at w/2 ± this)
...
function buildCushionSegs(): CushionSeg[] {
  ...
  // Inset each felt segment by R along its inward normal -> the ball-centre
  // locus the physics reflects off. Keep the felt endpoints around so we can
  // miter neighbours below.
  const segs = felt.map(({ a, b, ref }) => {
    ...
    return {
      fa: a, // felt endpoints (shared vertices key off these)
      fb: b,
      a: P(a.x + nx * R, a.y + ny * R), // inset endpoints
      b: P(b.x + nx * R, b.y + ny * R),
      nx,
      ny,
      tx,
      ty,
  ...
  // Miter shared corners. Insetting each face independently splits a felt vertex
  // into two inset endpoints; at a convex corner that leaves a GAP a ball centre
  // slips through (the jaws were being ignored). Snap both inset endpoints of a
  // shared vertex to where their inset lines cross — closes convex gaps, trims
  // concave overlaps. Genuine pocket mouths (unshared vertices) stay open.
  const vkey = (v: Vec) => `${v.x.toFixed(4)},${v.y.toFixed(4)}`;
  const corners = new Map<string, { i: number; end: "a" | "b" }[]>();
  segs.forEach((s, i) => {
    for (const end of ["a", "b"] as const) {
      const k = vkey(end === "a" ? s.fa : s.fb);
  ...
  for (const list of corners.values()) {
    if (list.length !== 2) continue; // mouth gap or T-junction: leave as-is
    const [p, q] = list;
    const sp = segs[p.i];
    const sq = segs[q.i];
    const pp = sp[p.end]; // inset point on seg p's line
    const qp = sq[q.end];
    // Intersect line(pp, dir=sp tangent) with line(qp, dir=sq tangent).
    const denom = sp.tx * -sq.ty - -sq.tx * sp.ty;
    if (Math.abs(denom) < 1e-9) continue; // near-parallel: no sensible miter
    const s = ((qp.x - pp.x) * -sq.ty - -sq.tx * (qp.y - pp.y)) / denom;
    const m = P(pp.x + sp.tx * s, pp.y + sp.ty * s);
    sp[p.end] = m;
    sq[q.end] = m;
  }

  return segs.map((s) => {
    const len = Math.hypot(s.b.x - s.a.x, s.b.y - s.a.y) || 1e-9;
    return { a: s.a, b: s.b, nx: s.nx, ny: s.ny, tx: s.tx, ty: s.ty, len };
  });
}
...
/**
 * Cushion rebound off an arbitrary polygon segment: reflect the normal velocity
 * with restitution, apply capped Coulomb friction from the tangential surface
 * speed (which includes side spin). For an axis-aligned nose this is byte-for-
 * byte identical to the old per-rail code; the angled jaws reuse the same maths.
 */
function resolveSeg(
  b: Ball,
  s: CushionSeg,
  ...
  const vn = b.v.x * s.nx + b.v.y * s.ny; // < 0 while approaching the face
  ...
  for (let iter = 0; iter < 32 && remaining > 1e-9; iter++) {
    ...
    for (const b of balls) {
      ...
      for (const s of CUSHION_SEGS) {
        ...
        const t = -d0 / vn; // time for the centre to reach the segment line
